...
class NetworkToolkit:
    ...
    def boost_current_process_priority(self):
        """
        ...
        try:
            ...
            from linux_kernel_bridge import require_linux
        except Exception as exc:
            logger.warning(f"Linux priority boost helper is unavailable: {exc}")
            return None

        try:
            require_linux()
        except Exception as exc:
            logger.warning(f"Skipping priority boost on non-Linux host: {exc}")
            return None
...
def main():
    ...
    # Best-effort priority boost for this process on Linux.
    toolkit.boost_current_process_priority()
